...
class Visualization(Prediction):
    ...
    def technical_analysis_graph(self):
        self.df_visulization_technical = self.df.iloc[-750:]
        self.fig_analysis = make_subplots(rows = 4, cols = 1)

        self.fig_analysis.append_trace(go.Scatter(x = self.df_visulization_technical.index, y = self.df_visulization_technical['OBV'], name = "OBV", 
        marker = dict(color = '#708090')), row = 1, col = 1)
        ...
        marker = dict(color = '#2ECC71')), row = 2, col = 1)
        ...
        marker = dict(color = '#E74C3C')), row = 2, col = 1)
        ...
        marker = dict(color = '#000000')), row = 2, col = 1)
        self.fig_analysis.add_shape(type = 'line', x0 = self.df_visulization_technical.index.min(), x1 = self.df_visulization_technical.index.max(), 
        y0 = 0, y1 = 0, line = dict(color = '#000000', width = 0.5), row = 2, col = 1)
        ...
        self.fig_analysis.add_shape(type = 'line', x0 = self.df_visulization_technical.index.min(), x1 = self.df_visulization_technical.index.max(), y0 = 20, y1 = 20, line = dict(color = '#008000', width = 1), row = 4, col = 1)
        self.fig_analysis.add_shape(type = 'line', x0 = self.df_visulization_technical.index.min(), x1 = self.df_visulization_technical.index.max(), y0 = 80, y1 = 80, line = dict(color = '#FF0000', width = 1), row = 4, col = 1)
        ...
        self.fig_analysis.update_yaxes(title_text = "OBV", row = 1, col = 1)
        self.fig_analysis.update_yaxes(title_text = "MACD", row = 2, col = 1)
